        ...
        <div class="register-container_form">
            ...
            <form action="{{ route('payment.process') }}" method="POST" class="payment-form" id="payment-form" onsubmit="return validateExpiryDate()">
                ...
                <input
                    ...
                    value="{{ old('card_name') }}"
                    pattern="^[A-Za-z\s]+$"
                    title="El nombre solo puede contener letras sin acentos y espacios, con un máximo de 50 caracteres."
    ...
    <script>
        ...
        function validateCardName(input) {
            input.value = input.value
                .replace(/[^A-Za-z\s]/g, '')
                .slice(0, 50); 
            document.getElementById('display-card-name').textContent = input.value || 'NOMBRE APELLIDO';
        }
